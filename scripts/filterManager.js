export class FilterManager {
	...
	applyFilters(ctx, render) {
		...
		if (hasFilters) {
			...
			const hueRotate =
				this.hueDegrees > 0 ? `hue-rotate(${this.hueDegrees}deg) ` : "";
			const invert =
				this.invertPercent > 0 ? `invert(${this.invertPercent}%) ` : "";
			const opacity =
				this.opacityPercent < 100
					? `opacity(${this.opacityPercent}%) `
					: "";
			const blur =
				this.blurPixels > 0 ? `blur(${this.blurPixels}px) ` : "";
			const brightness =
				this.brightnessPercent !== 100
					? `brightness(${this.brightnessPercent}%) `
					: "";
